...
class Deployer {
    ...
    constructor(apiUrl, /* see krono-coin postDeploy */ userId, /* see krono-coin postDeploy */ keyPair, /* see krono-coin postDeploy */ run, rootPath, env, modulePath = path.join(rootPath, 'node_modules'), debug = true, skipGit = false) {
        this.apiUrl = apiUrl;
        this.userId = userId;
    ...
    async deploy(source, crumbs = 'root', depth = 0) {
        ...
        let presets;
        try {
            const { data } = await fyx_axios_1.default.get(`${this.apiUrl}/chains/${chainFilePath}`);
            presets = data;
        }
        catch (e) {
            if (e.status !== 404)
